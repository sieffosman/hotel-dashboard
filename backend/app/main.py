...
async def upload_temp_image(image: UploadFile = File(...)):
    ...
    try:
        ...
        temp_filename = f"temp_{datetime.now().timestamp()}_{uuid.uuid4().hex[:8]}{file_extension}"
...
async def finalize_room_image(room_id: int, request: dict, session=Depends(get_session)):
    ...
    try:
        ...
        permanent_filename = f"room_{room_id}_{int(datetime.now().timestamp())}{file_extension}"
...
def update_existing_room(*, session=Depends(get_session), room_id: int, room_in: RoomUpdate):
    room = get_room(session, room_id)
    if not room:
        raise HTTPException(404, 'Room not found')
...
def generate_room_pdf_endpoint(*, session=Depends(get_session), room_id: int):
    ...
    try:
        ...
        return Response(
            ...
            headers={"Content-Disposition": f"attachment; filename=room_{room_id}_{room.name.replace(' ', '_')}.pdf"}
        )
    except Exception as e:
